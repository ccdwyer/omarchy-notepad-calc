...
| `tests/ui/run.sh` | Loads **Panel.qml**, `grabToImage` on the **chrome Item**, writes 12 PNGs to a **temp dir**. If committed goldens exist, pixel-diffs against them (synthetic stand-ins fail). If they are **absent**, skip-with-warning (this Mac cannot bootstrap). `UPDATE_UI_GOLDENS=1` writes Linux grabToImage captures for commit. |
...
Golden PNGs must be Linux `Item.grabToImage` captures of `Panel.qml`. There is no stand-in generator. **If `tests/goldens/ui/*.png` are missing**, Linux CI runs `UPDATE_UI_GOLDENS=1`, uploads the PNGs as artifact `notepad-calc-ui-captures`, and **skips pixel-diff with a warning** (not a hard fail, and not a same-run tautology). **Once those PNGs are committed**, subsequent CI runs pixel-diff a fresh temp capture against the git baselines (2% AE; stand-ins fail). Bootstrap: download the artifact, `cp` into `tests/goldens/ui/`, commit. This Mac cannot produce those grabs.
