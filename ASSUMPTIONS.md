...
- **IPC.** `omarchy-shell shell call <id>` hits overlay/panel loaders only; this plugin is bar-widget-only so that returns `unknown`. Documented keybinds use the bar-widget `IpcHandler`: `omarchy-shell io.github.chris.notepad-calc toggle '{}'`. The root still defines string-argument `summon` / `hide` / `toggle` (delegating to `open` / `close`). Host-level `shell summon|hide|toggle <id>` is for panel/overlay kinds and is not used here.
