...
| `tests/ui/run.sh` | Loads **Panel.qml**, `grabToImage` on the **chrome Item**, writes 12 PNGs to a **temp dir**. If committed goldens exist, pixel-diffs against them. If they are **absent**, macOS skip-warns; Linux CI (`REQUIRE_QML_UI=1`) generates captures for artifacts then **fails** until the PNGs are committed. |
...
Golden PNGs must be Linux `Item.grabToImage` captures of `Panel.qml`. There is no stand-in generator. **If `tests/goldens/ui/*.png` are missing**, Linux CI generates them (`UPDATE_UI_GOLDENS=1`), uploads artifact `notepad-calc-ui-captures`, and **fails the job** so the skip cannot stay green forever. Bootstrap: download the artifact, `cp` into `tests/goldens/ui/`, commit. **Once committed**, CI pixel-diffs a fresh temp capture against git (2% AE). This Mac cannot produce those grabs.
